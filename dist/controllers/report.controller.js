...
exports.getDashboardDetailed = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
const getDashboardDetailed = async (req, res) => {
    try {
        ...
        const salesByMonth = monthLabels.map(label => {
            ...
            const monthOrders = recentOrders.filter((o) => new Date(o.createdAt).toLocaleString('default', { month: 'short' }) === label);
            return monthOrders.reduce((sum, o) => sum + Number(o.total), 0);
        });
        ...
        const pieValues = categoryDistribution.slice(0, 3).map((c) => c._count.id);
...
exports.getDashboardDetailed = getDashboardDetailed;
